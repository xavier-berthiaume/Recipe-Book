...
QVariant IngredientListModel::data(const QModelIndex &index, int role) const {
  if (!index.isValid() || index.row() >= m_ingredients.size())
    return QVariant();
...
void IngredientListModel::populate(const QList<QIngredient *> &list) {
  ...
  for (QIngredient *ingredient : list) {
    ...
    connect(ingredient, &QIngredient::descriptionChanged, this,
            [this, ingredient]() {
              int row = m_ingredients.indexOf(ingredient);
              if (row >= 0) {
                QModelIndex index = this->index(row);
...
void IngredientListModel::modifyIngredient(int index, const QVariant &data,
                                           int role) {

  QModelIndex indx = this->index(index);
  if (!indx.isValid() || index >= m_ingredients.size())
    return;
...
void IngredientListModel::removeIngredient(int index) {
  if (index < 0 || index >= m_ingredients.size())
    return;
...
void IngredientListModel::ingredientCreated(QIngredient *ingredient) {
  ...
  qDebug() << "Added ingredient to IngredientListModel with name "
           << ingredient->getName();
}

QIngredient *IngredientListModel::getIngredient(const QModelIndex &index) {
  if (!index.isValid() || index.row() >= m_ingredients.size())
    return nullptr;

  return m_ingredients[index.row()];
}

QIngredient *IngredientListModel::getIngredient(int index) {
  if (index < 0 || index >= m_ingredients.size())
    return nullptr;
