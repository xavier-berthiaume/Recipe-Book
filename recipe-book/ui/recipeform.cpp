...
void RecipeForm::initialize() {
  ...
  connect(m_instructionEdit, &QLineEdit::returnPressed, this,
          &RecipeForm::addInstructionToList);
  connect(m_equipmentCreate, &QToolButton::clicked, this,
          &RecipeForm::addEquipmentToList);
  connect(m_equipmentEdit, &QLineEdit::returnPressed, this,
          &RecipeForm::addEquipmentToList);
  ...
  connect(this, &QDialog::accepted, [this]() {
    ...
    extractFormData();
    if (isEditMode()) {
      qDebug() << "Submitted form to update recipe" << m_data["id"].toString();
...
void RecipeForm::on_createRecipeIngredientButton_clicked() {
  m_ingredientForm = RecipeIngredientForm::createForm(this);

  connect(m_ingredientForm, &RecipeIngredientForm::createRequested, this,
          &RecipeForm::createRequested);
  connect(m_ingredientForm, &RecipeIngredientForm::requestObject, this,
          &RecipeForm::requestObject);
  connect(m_ingredientForm, &RecipeIngredientForm::requestObjects, this,
          &RecipeForm::requestObjects);
  connect(m_ingredientForm, &RecipeIngredientForm::requestObjectsCount, this,
          &RecipeForm::requestObjectsCount);

  m_ingredientForm->exec();

  // After form is completed
  m_ingredientForm = nullptr;
}
