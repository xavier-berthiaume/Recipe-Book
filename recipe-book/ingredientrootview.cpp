...
IngredientRootView::IngredientRootView(DataCache *cache, QWidget *parent)
    : QWidget(parent), ui(new Ui::IngredientRootView), m_cache(cache),
  ...
  m_model->populate(m_cache->getIngredientCache());
  ...
  connect(delegate, &IngredientListDelegate::editClicked, this,
          &IngredientRootView::editButtonClicked);
  connect(delegate, &IngredientListDelegate::deleteClicked, this,
          &IngredientRootView::deleteButtonClicked);

  // Connect factory creation function
  connect(m_factory, &IngredientFactory::ingredientCreated, this,
          &IngredientRootView::ingredientCreated);
}

IngredientRootView::~IngredientRootView() { delete ui; }
...
void IngredientRootView::on_createButton_clicked() {
  ...
  QPlainTextEdit *descriptionEdit =
      this->findChild<QPlainTextEdit *>("descriptionEdit");
...
void IngredientRootView::on_cancelButton_clicked() {
  ...
  QPlainTextEdit *descriptionField =
      findChild<QPlainTextEdit *>("descriptionEdit");
...
void IngredientRootView::on_nameLineEdit_textChanged(const QString &arg1) {
  ...
  confirmButton->setEnabled(!arg1.isEmpty() &&
                            !descriptionField->toPlainText().isEmpty());
}
...
void IngredientRootView::on_descriptionEdit_textChanged() {
  ...
  confirmButton->setEnabled(!nameField->text().isEmpty() &&
                            !descriptionField->toPlainText().isEmpty());
}
...
void IngredientRootView::on_confirmButton_clicked() {
  ...
  if (m_editMode && m_editIngredient.isValid()) {
    m_model->modifyIngredient(m_editIngredient.row(), QVariant(name),
                              IngredientListModel::NameRole);
    m_model->modifyIngredient(m_editIngredient.row(), QVariant(description),
                              IngredientListModel::DescriptionRole);
  } else {
    m_factory->createIngredient(m_cache->getSelectedProfile(), name,
                                description);
  }
...
void IngredientRootView::editButtonClicked(const QModelIndex &index) {
  ...
  descriptionEdit->setPlainText(
      index.data(IngredientListModel::DescriptionRole).toString());
...
void IngredientRootView::deleteButtonClicked(const QModelIndex &index) {
  ...
  if (confirmationDialog->exec() == QDialog::Accepted) {
    ...
    QMessageBox::information(this, tr("Ingredient Deleted"),
