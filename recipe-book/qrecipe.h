...
class QRecipe : public QIngredient {
  ...
  Q_PROPERTY(quint32 prepTime READ getPrepTime WRITE setPrepTime NOTIFY
                 prepTimeChanged)
  Q_PROPERTY(QList<QRecipeIngredient *> m_ingredients READ getIngredients WRITE
                 setIngredients NOTIFY ingredientsChanged)
...
public:
  ...
  explicit QRecipe(const QUuid &id, QObject *parent = nullptr);
  explicit QRecipe(const QString &name, const QString &description,
                   ...
                   quint32 prepTime, QObject *parent = nullptr);
