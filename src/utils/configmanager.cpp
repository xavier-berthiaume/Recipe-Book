...
auto ConfigManager::get_config_path() -> std::string
{
...
auto ConfigManager::init() -> ConfigManager &
{
...
auto ConfigManager::getInstance() -> ConfigManager &
{
  if (m_instance == nullptr)
  {
...
auto ConfigManager::hasKey(const std::string &section, const std::string &key)
    -> bool
{
...
auto ConfigManager::getSections() const -> std::vector<std::string>
{
  std::vector<std::string> groups;

  for (const auto &group : m_keyfile->get_groups())
  {
...
auto ConfigManager::getKeys(const std::string &section) const
    -> std::vector<std::string>
{
  ...
  for (const auto &key : m_keyfile->get_keys(section))
  {
    keys.push_back(key);
  }
